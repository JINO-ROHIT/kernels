...
__global__ void vector_add(half* __restrict__ x, half* __restrict__ y, half* __restrict__ z, const int num){
    ...
    auto tile_rx_h2 = recast<half2>(tile_rx);
    auto tile_ry_h2 = recast<half2>(tile_ry);
    auto tile_rz_h2 = recast<half2>(tile_rz);

    #pragma unroll
    for (int i = 0; i < size(tile_rx); i++)
    {
        tile_rz_h2(i) = tile_rx_h2(i) + tile_ry_h2(i);
    }

    auto tile_rz_normal = recast<half>(tile_rz_h2);

    // move back to global
    copy(tile_rz_normal, tile_gz);
}
...
int main(){
    ...
    std::cout << "vector add cute version faster took " << elapsedTime / 100 << "ms." << std::endl;
}
