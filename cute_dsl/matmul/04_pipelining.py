"""
prefill 2 K-tiles into smem
for each K-tile:
    compute on current smem tile
    while computing, launch async load of a future K-tile into the free smem stage
    while computing each mma fragment, prefetch the next fragment from smem to registers
"""
import cutlass
...
def sgemm_kernel(
    ...
    # we can also use zipped divide btw, check concepts/05_local_tile
    ...
    # shared mem has 3 stages
    # stage 0  [ empty ]  we will will write here first
    # stage 1  [ empty ]
    # stage 2  [ empty ]

    #copy k tiles upto (3-1) from gmem to smem stage 0
    ...
    """
    till here we have 
    - stage 0 in smem already has K-tile 0
    - stage 1 in smem already has K-tile 1
    - stage 2 is free for the next gmem load
    - registers tCrA/tCrB already hold the first MMA fragment of smem tile 0
    """
    # ======================================================================
    # Mainloop.
    for _ in range(num_tiles_k, unroll_full=False):
        for mma_k_idx in range(num_mma_k, unroll_full=True):
            ...
            # which fragment to load next
            """
             - if mma_k_idx = 0, then mma_k_next = 1
            - if mma_k_idx = 3, then mma_k_next = 4
            - if mma_k_idx = 7, then mma_k_next = 0
            """
            mma_k_next = (mma_k_idx + 1) % num_mma_k
            """
            this is the core register prefetch idea:

            - current iteration computes with tCrA[..., mma_k_idx], tCrB[..., mma_k_idx]
            - before that compute, it loads tCrA[..., mma_k_next], tCrB[..., mma_k_next]

            So compute is always one fragment behind the smem->register load.
            """
...
def sgemm_host(a: cute.Tensor, 
    ...
    # index = tile_m * 1 + tile_k * (tile_m + padding_a) + num_stages * (tile_k * (tile_m + padding))
    # 1. to move along M , you just need to move one step
    ...
    print(f"total shared memory needed: {smem_size//1024} kb")
