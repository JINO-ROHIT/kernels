...
def sgemm_kernel(
    ...
    for tile_k_idx in range(num_tiles_k, unroll_full=False):
        ...
        cute.arch.barrier() # block wide synchronization like __syncthreads
        ...
        cute.arch.barrier() # block wide synchro
...
def sgemm_host(a: cute.Tensor, 
    ...
    smem_layout_a = cute.make_layout(
        shape=(tile_m, tile_k), stride=(1, tile_m)
    )
    smem_layout_b = cute.make_layout(
        shape=(tile_n, tile_k), stride=(1, tile_n)
    )
    smem_size = sum(
    ...
    A tile is:
    (tile_m, tile_k) = (128, 8)
    and the copy partition expects a thread layout that maps cleanly onto that shape. (32, 8)
    the second mode matches exactly
    """
    thr_layout_a = cute.make_ordered_layout(shape=(threads // tile_k, tile_k), order=(1, 0)) # (32, 8) should equal the 256 threads we launch
