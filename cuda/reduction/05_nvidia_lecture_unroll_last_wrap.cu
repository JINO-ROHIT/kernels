...
__device__ void warpReduce(volatile float* cache, unsigned int tid){ // we need volatile keyword here
...
__global__ void reduce_v1(float* __restrict__ input, float* __restrict output){
    ...
    for(int stride = blockDim.x/2; stride > 32; stride /= 2){ // for the last warp we do it manually 
        if(tid < stride){
