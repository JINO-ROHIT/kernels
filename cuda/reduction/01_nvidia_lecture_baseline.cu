...
__global__ void reduce_v0(float* __restrict__ input, float* __restrict output){
    ...
    for(int stride = 1; stride < blockDim.x; stride *= 2){
        ...
        __syncthreads(); // put this outside the for loop and check the bug in the sums calculated
    }

    if(tid == 0){
        output[blockIdx.x] = smem[0]; // each block stores its own reduced value
    }
}
